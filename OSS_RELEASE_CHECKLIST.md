...
- [/] **Phase 2: 再現性担保（Portability）** `[60%]`
- [ ] **Phase 3: エコシステム構築（Public Release）** `[20%]`
...
- [ ] **Docker Composeによる統合デモ環境構築**
  - **内容:** `docker compose up` 一発で、Ollama（LLM自動プル）＋ `tanuki-serving` ＋ シミュレーション用 UI のセットが立ち上がる環境の構築。
  - **状況:** 現状は `tanuki-serving` 単体の Dockerfile/Compose のみ。ローカルLLM初期化とUIの自動バインドを含むフルパッケージ化が必要。
- [ ] **テスト自動化とカバレッジ確保**
  - **状況:** 手動の SDK 疎通テスト（`test_sdk_api.py`）はあるが、CI/CDで確実に動作する pytest 等のテスト自動化スイートの拡充が必要。
