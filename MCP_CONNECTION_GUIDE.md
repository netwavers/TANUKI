...
# 標準運用: tanuki-journal write 末尾の run_tanuki_compiler()
```

Dockerで動いている場合、ボリュームマウントにより知識.dbが自動反映されます。

**既知の弱点（必読）**: 開発日誌と MCP の取得タイミング・データ源のずれは  
`TANUKI/docs/KNOWLEDGE_BASE_LIMITATIONS.md` にまとめています。MCP が空でもローカル md は存在しうるため、直近確認は `read_file` を併用してください。

**MCP 用 factory 同期（compiler 後）**:

```powershell
tanuki-journal sync
# または
D:\Projects\PyProjects\TANUKI\scripts\sync_kb_to_factory.ps1
```
