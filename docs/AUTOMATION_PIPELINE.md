...
### ファイル
- `scripts/drive_to_tanuki.ps1` …… メインスクリプト
...
# 基本実行（Git commit + push + compile + factory sync）
.\scripts\drive_to_tanuki.ps1

# オプション
.\scripts\drive_to_tanuki.ps1 -SkipCompile          # コンパイルを飛ばす
.\scripts\drive_to_tanuki.ps1 -SkipFactorySync      # factory sync を飛ばす
.\scripts\drive_to_tanuki.ps1 -DryRun               # 実際には何も変更しない
.\scripts\drive_to_tanuki.ps1 -Force                # エラーでも続行
```
...
2. 変更した `.md` ファイルをローカルの  
   `D:\Projects\PyProjects\TANUKI\documents\specs\`  
   にコピー（またはダウンロード）する。
...
[4] scripts/sync_kb_to_factory.ps1 実行（WAL checkpoint 込み）
[5] 完了ログ出力
```
